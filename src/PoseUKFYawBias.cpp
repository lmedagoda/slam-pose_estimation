...
PoseUKFYawBias::PoseUKFYawBias() : dirty(true)
{
...
//     MTK::setDiagonal(process_noise_cov, &WPoseState::angular_velocity, 0.00001);
    ...
    process_noise_cov.block(0,0,3,3) = 0.01 * base::Matrix3d::Identity();
      ...
      process_noise_cov.block(6,6,3,3) = 0.00001 * base::Matrix3d::Identity();
      process_noise_cov.block(9,9,3,3) = 0.00001 * base::Matrix3d::Identity();
      process_noise_cov.block(12,12,1,1) = 0.0003 * base::Matrix3d::Identity();
}

void PoseUKFYawBias::setInitialState(const base::samples::RigidBodyState& body_state)
{
    dirty = true;
...
void PoseUKFYawBias::setProcessNoiseCovariance(const Covariance& noise_cov)
{
    process_noise_cov = noise_cov;
}

void PoseUKFYawBias::predictionStep(const double delta)
{
    if(ukf.use_count() == 0)
    {
...
void PoseUKFYawBias::correctionStep(const Measurement& measurement)
{
    if(ukf.use_count() == 0)
    {
	setInitialState(body_state);
...
const base::samples::RigidBodyState& PoseUKFYawBias::getCurrentState()
{
    if(dirty)
...
void PoseUKFYawBias::rigidBodyStateToUKFState(const base::samples::RigidBodyState& body_state, PoseUKFYawBias::WPoseState& state, ukfom::ukf< PoseUKFYawBias::WPoseState >::cov& covariance)
{
...
void PoseUKFYawBias::UKFStateToRigidBodyState(const PoseUKFYawBias::WPoseState& state, const ukfom::ukf< PoseUKFYawBias::WPoseState >::cov& covariance, base::samples::RigidBodyState& body_state)
{
